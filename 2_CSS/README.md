...
<head>
  ...
  <!-- link : 닫는 태그 없음. 외부에 있는 스타일 시트 불러올 때 사용 -->
</head>
```
...
- !important > 인라인 방식 > 아이디 선택자(#id) > 클래스 선택자(.class) > 태그 선택자 > 전체 선택자
  <!-- 선택 적용 범위를 줄여갈수록 우선순위가 높아짐 -->
...
- 상대 크기
  ...
  - **rem** : 최상위 요소(`html`)에서 지정한 폰트의 크기를 1rem으로 계산 <!-- 1rem = 16px -->
...
```
rgb(red(0~255), green(0,255), blue(0~255)); /* (0, 0, 0) ~ (255, 255, 255) */
...
/* 많이 사용하게 되는 것 : center, space-between */
```
