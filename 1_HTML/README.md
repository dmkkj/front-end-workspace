...
```html
<meta charset="UTF-8" />
<!-- 문자 세트 지정 --><!-- "UTF-8" 전세계적, 보편적으로 쓰는 방식 -->
...
- action : 데이터를 전달할 서버를 지정 <!-- 지금은 사용할 수 없음 -->
- method : 데이터 전송 방식을 지정 <!-- get, post -->
- name : form 태그의 이름을 지정

### 전송 방식 : get, post <!-- *기술 면접에서도 물어보는 질문 -->

- get <!-- '검색'하고 검색 결과 데이터를 가지고 오기 때문에 불러올 때 사용 -->
  - 데이터를 URL에 노출시켜서 서버에 전달한다.
  - 데이터의 안전성이 낮다. (노출로 인해)
  - 데이터를 불러올 때 사용한다.
- post <!-- '회원가입' 시 정보를 전송하고 끝 -->
  - 데이터를 보낼 때 사용한다.
...
4. **radio** : 여러 항목 중 한 가지만 선택할 수 있다. <!-- checkbox와 radio 둘 다 선택을 하는 것은 같음 -->
5. **file** : 파일을 첨부할 수 있다.
